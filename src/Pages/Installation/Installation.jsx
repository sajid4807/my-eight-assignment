...
const Installation = () => {
  ...
  const handleSort = (type) => {
    setSort(type);
    if (type === "ratingAvg") {
      const sortedRatings = [...installList].sort(
  ...
  useEffect(() => {
    ...
    const installApp = data.filter((app) =>
      convertedInstallAppData.includes(app.id)
    );
    ...
    <div className="max-w-[1435px] mx-3 lg:mx-auto mb-10 lg:mb-20">
      ...
      <div className="flex justify-between items-center mb-3">
        <p className="text-2xl font-semibold text-[#001931]">
          <span>{installList.length}</span> Apps Found
        </p>
        <details className="dropdown dropdown-bottom dropdown-end">
          <summary className="btn bg-white m-1">Sort By Rating</summary>
          <ul className="menu dropdown-content bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm">
            <li>
              <a onClick={() => handleSort("ratingAvg")}>Low-High</a>
            </li>
            <li>
              <a onClick={() => handleSort("rating")}>High-Low</a>
            </li>
          </ul>
        </details>
      </div>
